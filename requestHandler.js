...
var requestHandler = function (fs, mongoose, event, dbsArray) {
    var logWriter = require("./Modules/additions/logWriter.js")(fs),
        ...
        instructor = require("./Modules/Instructor.js")(logWriter, mongoose, employee, role, models, record, classes, personTree, jobPosition, event);
...
//---------------------Classes---------------------------------------------
    ...
    function addClass(req, res, data) {
        if (req.session && req.session.loggedIn && req.session.lastDb) {
            access.getEditWritAccess(req, req.session.uId, 42, function (access) {
                if (access) {
                    classes.createData(req, res, data);
                } else {
                    res.send(403);
                }
            });
    ...
    function updateClass(req, res, data) {
        if (req.session && req.session.loggedIn && req.session.lastDb) {
            access.getEditWritAccess(req, req.session.uId, 42, function (access) {
                if (access) {
                    classes.updateData(req, res, data);
                } else {
                    res.send(403);
                }
            });
    ...
    function deleteClass(req, res, data) {
        if (req.session && req.session.loggedIn && req.session.lastDb) {
            access.getEditWritAccess(req, req.session.uId, 42, function (access) {
                if (access) {
                    classes.deleteData(req, res, data);
                } else {
                    res.send(403);
                }
            });
...
//-------------------------Instructor-----------------------------------
    ...
    function getInstructorByCode(req, res, data){
        if (req.session && req.session.loggedIn && req.session.lastDb) {
            access.getReadAccess(req, req.session.uId, 42, function (access) {
                if (access) {
                    instructor.getDataByCode(req,res,data);
                } else {
                    res.send(403);
                }
            });
    ...
    function getInstructorNew(req, res){
        if (req.session && req.session.loggedIn && req.session.lastDb) {
            access.getReadAccess(req, req.session.uId, 42, function (access) {
                if (access) {
                    instructor_coppy.getData(req, res);
                } else {
                    res.send(403);
                }
            });
    ...
    function getInstructorByCodeNew(req, res, data){
        if (req.session && req.session.loggedIn && req.session.lastDb) {
            access.getReadAccess(req, req.session.uId, 42, function (access) {
                if (access) {
                    instructor_coppy.getDataByCode(req, res, data);
                } else {
                    res.send(403);
                }
            });
...
//-----------------------------Rate----------------------------------------
    ...
    function getRateByInstructorCode(req, res, data){
        if (req.session && req.session.loggedIn && req.session.lastDb) {
            access.getReadAccess(req, req.session.uId, 42, function (access) {
                if (access) {
                    rate.getByIntructorCode(req, res, data);
                } else {
                    res.send(403);
                }
            });
...
//-----------------------------Teaching_Record-------------------------------
  //>old
    ...
    return {
        ...
        getTeachingRecordByCode: getTeachingRecordByCode,
