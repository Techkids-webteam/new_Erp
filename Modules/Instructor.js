var Instructor = function (logWriter, mongoose, employee, role, models, record, classes, personTree){
        ...
        models.get(req.session.lastDb - 1, "Instructor", instructorSchema)
          ...
          .exec(function(err, doc) {
            if(err) {
              ...
              res.json(400, {mess: 'Not found Instructor with code: ' + data})
            } else {
              res.json(200, doc);
            }
          });
    ...
    function createData(req, res, data) {
        ...
        var cbSave = function (employee_id) {
            newIns.save(function (err, result) {
                if(err) logWriter.log(err);
                else res.json(200, {id: result._id, result_code: 1, result_message: "success" });
            })
        };

        var cbCheck = function(employee_id) {
          newIns.employee_id = new ObjectId(employee_id);
          models.get(req.session.lastDb - 1, "Employees", employeeSchema)
            ...
            .exec(function(err, doc) {
              if(err) {
                ...
                res.json(400, 'Instructor.employee_id Not Found!');
              } else {
                cbSave(employee_id);
              }
            })
        ...
        if(data.employee_id) {
          cbCheck(data.employee_id);
        } else {
          employee.createData(req, data, res, cbSave)
        }
    }
