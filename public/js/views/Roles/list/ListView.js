define([
    'text!templates/Roles/list/ListHeader.html',
    'views/Roles/CreateView',
    'views/Roles/list/ListItemView',
    'collections/Roles/filterCollection',
    'models/RoleModel',
    ...
    'text!templates/stages.html'
       ...
       function (listTemplate, createView, listItemView, contentCollection, currentModel, editView, common, dataService, stagesTamplate) {
           var RolesListView = Backbone.View.extend({
               el: '#content-holder',
               ...
               goSort: function (e) {
                   ...
                   var target$ = $(e.target);
                   var currentParrentSortRoles = target$.attr('roles');
                   var sortRoles = currentParrentSortRoles.split(' ')[1];
                   var sortConst = 1;
                   var sortBy = target$.data('sort');
                   var sortObject = {};
                   if (!sortRoles) {
                       target$.addRoles('sortDn');
                       sortRoles = "sortDn";
                   }
                   switch (sortRoles) {
                   case "sortDn":
                       {
                           target$.parent().find("th").removeRoles('sortDn').removeRoles('sortUp');
                           target$.removeRoles('sortDn').addRoles('sortUp');
                           sortConst = 1;
                       }
                       break;
                   case "sortUp":
                       {
                           target$.parent().find("th").removeRoles('sortDn').removeRoles('sortUp');
                           target$.removeRoles('sortUp').addRoles('sortDn');
                           sortConst = -1;
                       }
                       break;
                   }
               ...
               getTotalLength: function (currentNumber, itemsNumber) {
                   dataService.getData('/totalCollectionLength/Roles', {
