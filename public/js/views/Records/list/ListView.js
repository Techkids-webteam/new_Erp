define([
    'text!templates/Records/list/ListHeader.html',
    'views/Records/CreateView',
    'views/Records/list/ListItemView',
    'collections/Records/filterCollection',
    'models/RecordModel',
       ...
       function (listTemplate, createView, listItemView, contentCollection, currentModel, editView, common, dataService, stagesTamplate) {
           var RecordsListView = Backbone.View.extend({
               ...
               goSort: function (e) {
                   ...
                   var target$ = $(e.target);
                   var currentParrentSortRecords = target$.attr('roles');
                   var sortRecords = currentParrentSortRecords.split(' ')[1];
                   var sortConst = 1;
                   var sortBy = target$.data('sort');
                   var sortObject = {};
                   if (!sortRecords) {
                       target$.addRecords('sortDn');
                       sortRecords = "sortDn";
                   }
                   switch (sortRecords) {
                   case "sortDn":
                       {
                           target$.parent().find("th").removeRecords('sortDn').removeRecords('sortUp');
                           target$.removeRecords('sortDn').addRecords('sortUp');
                           sortConst = 1;
                       }
                       break;
                   case "sortUp":
                       {
                           target$.parent().find("th").removeRecords('sortDn').removeRecords('sortUp');
                           target$.removeRecords('sortUp').addRecords('sortDn');
                           sortConst = -1;
                       }
                       break;
                   }
               ...
               getTotalLength: function (currentNumber, itemsNumber) {
                   dataService.getData('/totalCollectionLength/Records', {
